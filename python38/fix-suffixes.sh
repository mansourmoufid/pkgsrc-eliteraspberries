...
test -d bin && (
    ...
    for f in 2to3 idle pydoc python pyvenv; do
        for s in "" 3 3.8 -3 -3.8; do
            test -L ${f}${s} && rm ${f}${s}
        done
    done
    for f in 2to3 pyvenv; do
        test -f ${f} && mv ${f} ${f}-3
        test -f ${f}-3 && mv ${f}-3 ${f}-3.8
    done
    ...
    test -f python3-config && \
        mv python3-config python3.8-config
) || true
test -d lib && (
    cd lib
    for ext in dylib so; do
        ...
        test -f libpython3.${ext} && \
            mv libpython3.${ext} libpython3.8.${ext}
    done
...
test -d man/man1 && (
    ...
    test -f python3.1 && mv python3.1 python3.8.1
) || true
